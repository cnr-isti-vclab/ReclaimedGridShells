...
class Mesh:
    ...
    def make_on_mesh_shared_computations(self):
        ...
        self.cross_dirs = torch.linalg.cross(self.edge_directions, self.edge_normals)

    # Computes edge-face adjacency matrix and the positions of free boundary edges.
    def make_adjacency_matrices(self):
        edge_list = []
        edge_pos = []
        edge_is_on_boundary = torch.zeros(self.edges.shape[0], dtype=torch.bool, device=self.device)

        # Building edge flaps face matrix.
        for edge in range(self.edges.shape[0]):
            faces = torch.any(self.edges_per_face == edge, dim=1).nonzero().flatten()
        ...
        edge_is_not_fixed = torch.logical_or(torch.logical_not(self.vertex_is_red)[self.edges][:, 0], torch.logical_not(self.vertex_is_red)[self.edges][:, 1])
        edge_is_on_boundary = torch.logical_and(self.vertex_is_on_boundary[self.edges][:, 0], self.vertex_is_on_boundary[self.edges][:, 1])
        self.free_boundary_edges = torch.arange(len(self.edges), device=self.device)[torch.logical_and(edge_is_not_fixed, edge_is_on_boundary)]
    ...
    def face_areas_normals(vs, faces, normalize=True):
        ...
        face_normals = torch.linalg.cross(face_verts[:, 1, :] - face_verts[:, 0, :],
